...
class Comic:
    def __init__(self, comicName, index):
...
class StatSys:
    def __init__(self, urlLis):
        ...
        self.vote_times = [
            0
        ] * 4  # vote_times[] saves the number of voters who vote once, twice ...
        ...
        self.votedUserDict = (
            dict()
        )  # create a dict whose key is comicName, value is a list contained the users who voted
        self.newComicLis = []
        ...
        for i in range(len(urlLis)):
            ...
            for tag in main_content:
                ...
                if tmp[0] == "=":
                    ...
                    tmpComic = Comic(comicName=tmp[1:last_word_index], index=ind)
                    self.newComicLis.append(tmpComic)
                    ind = ind + 1
    ...
    def startVote(self):
        ...
        for i in range(self.num_voter):
            ...
            self.vote_times[len(vote) - 1] = (
                self.vote_times[len(vote) - 1] + 1
            )  # update vote_times list with determine the length of voted_num_lis
            for j in range(len(vote)):
                vote_ind = vote[j]
                # print(vote_ind)
                self.resDict[self.comicLis[vote_ind - 1]] = (
                    self.resDict[self.comicLis[vote_ind - 1]] + 1
                )
                # self.result[vote_ind-1] = self.result[vote_ind-1]+1

        sorted_all_results = sorted(
            self.resDict.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def checkRelation(self, num1, num2):  # return P(vote num1 and num2| vote num1)
        ...
        out = (
            ...
            + num2_comic_name
            + " is "
            + str(vote_for_num1_num2 / vote_for_num1)
        )
        print(out)
        ...
        for i in range(len(self.sortedresList)):
            ...
            if self.sortedresList[i][1] != buffer:
                output = (
                    str(rank)
                    + ". "
                    + self.sortedresList[i][0]
                    + " "
                    + str(self.sortedresList[i][1])
                )
                print(output)
                repeated_times = 1
            else:
                output = (
                    str(rank - repeated_times)
                    + ". "
                    + self.sortedresList[i][0]
                    + " "
                    + str(self.sortedresList[i][1])
